...
inline void fillHMS(unsigned seconds,struct tm* utc){
    ...
    utc->tm_min=minute%60;
    utc->tm_hour=minute/60;
};
...
TimeZone::TimeZone(const char* zonefile) : data_(new TimeZone::Data){
    ...
    if(!detail::readTimeZoneFile(zonefile,data_.get())){
        //hared_ptr的reset( )函数的作用是将引用计数减1，停止对指针的共享，除非引用计数为0，否则不会发生删除操作。
        data_.reset();
    }
}
/*
eastOfUtc表示UTC时间。中国内地的时间与UTC的时差为+8，也就是UTC+8。美国是UTC-5。
